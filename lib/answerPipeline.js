/// Words that mix Latin and Cyrillic letters — "deneг", "сuл" — are a known
/// slip when the instructions are in English and the prose in Russian:
/// the model switches script inside a word for a character or two. They
/// are invisible in a log and glaring on a phone. Returns the offenders.
function mixedScriptWords(text) {
  if (typeof text !== 'string') return [];
  const words = text.match(/[\p{L}]+/gu) || [];
  return words.filter(w => /[A-Za-z]/.test(w) && /[\u0400-\u04FF]/.test(w));
}
...
async function buildAskResponse({
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    // The reflection and prayer in the model's reply are written to the
    // person's situation, so only the reference is logged.
    logger.info?.(
      `[answerPipeline] Attempt ${attempt} selection: ${normalized ? `${normalized.book} ${normalized.chapter}:${normalized.verseStart}-${normalized.verseEnd}` : 'unparseable'}`
    );
    ...
    // The model chooses in the numbering the other translations share; a
    // translation that counts differently (the Synodal Psalter) is converted
    // here, so both the text fetched and the reference shown agree with it.
    const located = localizeReference
      ? { ...normalized, ...(await localizeReference(normalized, lang)) }
      : normalized;

    const verseText = collapseWhitespace(
      await fetchPassageText({ ...located, lang })
    );

    if (verseText) {
      const bookName = books?.[lang]?.[located.book - 1] || `Book ${located.book}`;
      return {
        reference: `${bookName} ${located.chapter}:${formatVerseSuffix(located.verseStart, located.verseEnd)}`,
        // Structured location as well, so the app can open the reader on
        // this chapter without parsing the display string back apart.
        bookCode: bookCodes?.[located.book - 1] ?? null,
        chapter: located.chapter,
        verseStart: located.verseStart,
        verseText,
